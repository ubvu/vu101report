...
if c(os)=="MacOSX"	{ // if working on my own laptop
	cd "/Volumes/HDD/SURFdrive/shared/VU101innovations/"
}
else	{ // if working at VU
	cd "C:/Users/mzn203/surfdrive/shared/VU101innovations/"
}

// ===================================== Prepare cleaned free entries file =====
// =============================================================================
insheet using "./01 data raw/KAGGLE/survey_cleaned.csv", ///
	clear names delim(",")
...
rename role research_role
lab var research_role "Q1: What is your research role?"

rename rolespeccl research_role_ifother
...
// In de BIG101-file zitten veel meer uitzonderingen; geen tijd om die allemaal na te pluizen...
// Misschien later nog eens kijken
gen role=research_role
lab var role "What is your research role? (cleaned)"
replace role="PhD student" if inlist(research_role_ifother, "Pulmonologist - PhD researcher", ///
	...
	"Assistant researcher", "PhD degree", "PhD student")
replace role="PhD student" if inlist(research_role_ifother, "PhD student, librarian")
replace role="Postdoc" if inlist(research_role_ifother, "researcher (with PhD, but not as a postdoc)")

rename countrycl country
lab var country "Q3: What is the country of your current or last affiliation?"

rename phys PhysicalSciences
...
rename engtech EngineeringTechnology
...
rename socec SocialSciencesEconomics
...
foreach tt of varlist tool_search_*	{
	if "`tt'"=="tool_search_other" | "`tt'"=="tool_search_other_orig"	continue
...
replace `varname'="1" if `varname'=="MS Excel"
replace `varname'="0" if mi(`varname')
destring `varname', replace
...
replace `varname'="1" if `varname'=="OSF (Open Science Framework)"
replace `varname'="0" if mi(`varname')
destring `varname', replace
...
replace `varname'="1" if `varname'=="MS Word"
replace `varname'="0" if mi(`varname')
destring `varname', replace
...
replace `varname'="1" if `varname'=="Scalar"
replace `varname'="0" if mi(`varname')
destring `varname', replace
...
replace `varname'="1" if `varname'=="working papers"
replace `varname'="0" if mi(`varname')
destring `varname', replace
...
replace `varname'="1" if `varname'=="ScienceOpen Posters"
replace `varname'="0" if mi(`varname')
destring `varname', replace
...
replace `varname'="1" if `varname'=="PubPeer"
replace `varname'="0" if `varname'=="PubPeer"
destring `varname', replace
...
// =============================================================================
// not very useful, only rename
foreach vv of varlist lang?	{
	rename `vv' tool_`vv'
	lab var tool_`vv' "Language specific tool"
}
...
rename developcl `varname'
lab var `varname' "Most important development in scholarly communication"

local varname supportOA
rename openacc `varname'
cap drop _tmp
gen _tmp=0 if `varname'=="I don't know" | mi(`varname')
replace _tmp=-1 if `varname'=="No"
replace _tmp=1 if `varname'=="Yes"
drop `varname'
rename _tmp `varname'
lab var `varname' "Do you support Open Access?"

local varname supportOS
rename opensci `varname'
cap drop _tmp
gen _tmp=0 if `varname'=="I don't know" | mi(`varname')
replace _tmp=-1 if `varname'=="No"
replace _tmp=1 if `varname'=="Yes"
...
foreach vv of varlist `r(varlist)'	{
	qui replace `vv'=trim(itrim(`vv'))
}


// check if all tool vars are 0/1
qui ds tool_*
local inlist `r(varlist)'
// macro list _inlist
qui ds tool_*other* tool_lang*
local outlist `r(varlist)'
// macro list _outlist
local vlist: list inlist-outlist
// macro list _vlist
...
log using "./01 data raw/KAGGLE/BIG101surveydata-codebook.log", replace name(codebook)
codebook
log close codebook
set linesize `line'
